...
// ─────────────────────────────────────────────────────────────────────────────
// 1. ERROR DEFINITION (MUST come BEFORE any mod declarations)
// ─────────────────────────────────────────────────────────────────────────────

use soroban_sdk::{contracterror, Address, Env, String, Symbol, Vec};
...
pub enum EscrowError {
    ...
    CommentTooLong = 43,
    IndexOutOfBounds = 44,
    MilestoneAlreadyReleased = 45,
    ContractIdOverflow = 46,
    ContractIdCollision = 47,
    TimelockNotElapsed = 48,
    InvalidProtocolParameters = 49,
    InvalidContractId = 50,
    MilestoneNotOverdue = 51,
    AlreadyCancelled = 52,
    BatchItemLimitExceeded = 53,
    EvidenceTooLong = 54,
    InvalidParticipants = 55,
    AlreadyApproved = 56,
    InsufficientApprovals = 57,
    EscrowCapExceeded = 58,
}
...
pub const MAX_REPUTATION_BATCH_SIZE: u32 = 10;
pub const MAINNET_PROTOCOL_VERSION: u32 = 1u32;
pub const PAGE_CEILING: u32 = 20;
...
pub use amount_validation::{
    accumulate_amounts, checked_available_balance, safe_add_amounts,
    safe_subtract_amounts, validate_deposit_amount, validate_milestone_amounts,
...
// ─────────────────────────────────────────────────────────────────────────────
// 7. MAIN CONTRACT IMPL (uses all submodules)
// ─────────────────────────────────────────────────────────────────────────────
...
impl Escrow {
    ...
    pub fn accept_client_migration(env: Env, contract_id: u32, new_client: Address) -> bool {
        Self::require_not_paused(&env);
        Escrow::accept_client_migration_impl(&env, contract_id, new_client)
    }

    pub fn has_pending_client_migration(env: Env, contract_id: u32) -> bool {
        Escrow::has_pending_client_migration_impl(&env, contract_id)
    }

    pub fn get_pending_client_migration(env: Env, contract_id: u32) -> PendingClientMigration {
        Escrow::get_pending_client_migration_impl(&env, contract_id)
    }
    ...
    pub fn release_milestone(
        ...
        let available = checked_available_balance(
    ...
    pub fn refund_unreleased_milestones(
        ...
        for idx in milestone_indices.iter() {
            ...
            if let Some(deadline) = milestone.deadline {
                if !Self::is_milestone_overdue(env.clone(), contract_id, idx) {
                    env.panic_with_error(EscrowError::MilestoneNotOverdue);
                }
            }
        ...
        let available_balance = checked_available_balance(
            contract.funded_amount,
            contract.released_amount,
            contract.refunded_amount,
        )
    ...
    pub fn get_contract_summary(env: Env, contract_id: u32) -> ContractSummary {
        ...
        let refundable_balance = checked_available_balance(
            contract.funded_amount,
            contract.released_amount,
            contract.refunded_amount,
        )
    ...
    pub fn cancel_contract(env: Env, contract_id: u32, client: Address) -> bool {
        ...
        let refund_amount = checked_available_balance(
            contract.funded_amount,
            contract.released_amount,
            contract.refunded_amount,
        )
    ...
    pub fn issue_reputation_batch(
        ...
        if items.len() > MAX_REPUTATION_BATCH_SIZE {
            env.panic_with_error(EscrowError::BatchItemLimitExceeded);
        }
        caller.require_auth();
        let mut i = 0;
        while i < items.len() {
